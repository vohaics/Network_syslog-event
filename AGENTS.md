...
- The Tera Term executable path is configurable in the UI (**Settings**) and persisted in `settings.json` (gitignored). Resolution order: UI path → `TERATERM_EXE` env → auto-detect. `/api/teraterm-status?auto=1` ignores the saved path so Detect can still find installs, and `&path=...` reports why one specific path does or does not resolve.
- Tera Term runs on the **dashboard host**, not the browser machine. A dashboard started inside WSL/a container/a server cannot see `C:\...` or launch Windows GUI programs even though the browser hits `127.0.0.1`. `resolve_exe` translates Windows paths to `/mnt/<drive>/...` when off-Windows and names the host OS in its failure reason, since "path is correct but not found" is otherwise very confusing.
