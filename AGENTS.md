...
- Vendor matters as much as transport. Cisco IOS streams syslog via `terminal monitor`; FortiOS cannot, so `monitor_device` switches to polling `execute log display` every `FORTIOS_POLL_INTERVAL` and de-duplicates lines. Vendor comes from the `vendor` field or is detected from the CLI prompt (`detect_vendor`).
- `ssh_client.diagnose_ssh` backs the dashboard's per-device **Test** button (TCP → banner → auth → prompt) and is the fastest way to tell a wrong-protocol problem from a credential or reachability problem.
- Paramiko legacy algorithm tuning filters against `Transport._kex_info` / `_cipher_info` / `_key_info`. Advertising algorithms the installed Paramiko does not implement breaks the handshake and shows up as a misleading "no banner" error.
