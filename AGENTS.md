...
- The Tera Term executable path is configurable in the UI (**Settings**) and persisted in `settings.json` (gitignored). Resolution order: UI path → `TERATERM_EXE` env → auto-detect. `/api/teraterm-status?auto=1` ignores the saved path so Detect can still find installs.
